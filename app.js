/* =====================================================================
   ...
   Aquí se maneja toda la lógica: Inyectar HTML, filtrar ropa, 
   carrito de compras, animación del carrusel y llamadas a WhatsApp.
===================================================================== */
...
let itemsPerPage = 12; // <-- EDITAR AQUÍ: Cuántos productos cargar a la vez
...
async function initApp() {
  try {
    ...
    // Carga filtros desde URL si existen
...
// ==========================================
// FUNCIONALIDADES DE EFECTOS VISUALES AL BAJAR PANTALLA
// ==========================================
...
// ==========================================
// LÓGICA CORE: CARRITO, WHATSAPP Y HORARIOS
// ==========================================
...
// ==========================================
// MODALES INFORMATIVOS (INFO DE LA TIENDA)
// ==========================================
...
// ==========================================
// BARRAS DE NAVEGACIÓN Y CARRITO
// ==========================================
...
function renderNavbar(data) {
  ...
  const linksHTML = data.links
    .map(
      (link) =>
        `<a href="${link.href}" class="block md:inline-block py-2 text-textMuted hover:text-primary transform hover:-translate-y-1 transition-all duration-300 text-base md:text-sm font-medium">${link.label}</a>`,
    )
    .join("");
...
// ==========================================
// CARRUSEL DE NOVEDADES (ANIMACIÓN SUAVE CON FLECHAS)
// ==========================================

// Motor de animación personalizado para deslizamiento súper sedoso (sin CSS snap)
window.moveCarousel = function (direction) {
  const track = document.getElementById("slider-track");
  // Previene múltiples clics mientras se está moviendo
  ...
  // Calcula el ancho exacto de una tarjeta más el espacio de separación (24px por gap-6)
  const step = card.offsetWidth + 24;
  const start = track.scrollLeft;
  const end = start + step * direction;
  const duration = 600; // Duración en milisegundos (600ms = lento y suave)
  ...
  function animate(currentTime) {
    ...
    // Easing (easeOutQuart): Arranca rápido y frena con mucha delicadeza
    ...
    if (progress < 1) {
      ...
      // Lógica de bucle infinito imperceptible (Teletransporte)
      ...
      track.dataset.animating = "false"; // Libera el botón para el siguiente clic
    }
  }
  requestAnimationFrame(animate);
};
...
function renderCarousel() {
  ...
  // ANCHOS MATEMÁTICOS:
  // w-[82vw] asegura 1 en móvil y se asoma la siguiente.
  // lg:w-[calc(25%-18px)] asegura EXACTAMENTE 4 tarjetas completas en PC (25% menos el espacio del gap).
  ...
  // Posiciona el carrusel en la segunda copia clonada al inicio para permitir retroceder
  setTimeout(() => {
    const track = document.getElementById("slider-track");
    if (!track) return;
    const card = track.querySelector(".carousel-item");
...
// ==========================================
// RENDERIZADO DE SECCIONES INICIALES
// ==========================================
...
function renderCategories(data) {
  ...
  const itemsHTML = sortedItems
    .map((item) => {
      ...
      // NUEVO BADGE: Esquina Inferior Izquierda, Fondo Blanco, Texto Negro y Primario.
      return `
